...
export default function AccountSettings() {
  ...
  return <div className="w-full space-y-5">
    <SeoMeta title="Hesap Ayarları | E-Storee" description="E-Storee hesap, güvenlik, bildirim ve gizlilik ayarlarınızı yönetin." canonicalPath="/profile/settings" robots="noindex,nofollow" />
    <h1 className="sr-only">Hesap Ayarları</h1>
    ...
    {activeTab === "account" && <section className={sectionClass}><h3 className="text-xl font-bold text-[#111]">Account Details</h3><p className="mt-1 text-sm text-[#777]">Manage your personal information and preferences.</p><div className="mt-6 flex items-center gap-4"><img src="https://i.pravatar.cc/150?img=12" alt="User Avatar" width={64} height={64} loading="lazy" className="h-16 w-16 rounded-full object-cover" /><div><button type="button" className="rounded-lg bg-[#b6349a] px-3 py-2 text-xs font-semibold text-white">Change Avatar</button><button type="button" className="ml-2 rounded-lg bg-[#f3f3f3] px-3 py-2 text-xs font-semibold text-[#666]">Remove Photo</button></div></div><div className="mt-6 grid gap-4 md:grid-cols-2">{field("firstName", "First Name")}{field("lastName", "Last Name")}{field("phone", "Phone Number", "tel")}{field("email", "Email Address", "email")}{field("dob", "Date of Birth", "date")}{field("language", "App Language")}{field("currency", "Currency")}</div></section>}
    {activeTab === "security" && <section className={sectionClass}>
      <h3 className="text-xl font-bold text-[#111]">Security & Password</h3>
      <p className="mt-1 text-sm text-[#777]">Update your password and secure your account.</p>
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <label className="space-y-1.5 text-sm font-semibold text-[#555]">New Password<input type="password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.currentTarget.value)} className={inputClass} /></label>
        <label className="space-y-1.5 text-sm font-semibold text-[#555]">Confirm New Password<input type="password" autoComplete="new-password" value={passwordConfirmation} onChange={(event) => setPasswordConfirmation(event.currentTarget.value)} className={inputClass} /></label>
      </div>
      <button type="button" disabled={passwordSaving} onClick={() => void handlePasswordUpdate()} className="mt-5 rounded-lg bg-[#111] px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">{passwordSaving ? "Updating..." : "Update Password"}</button>
      {passwordStatus && <p className="mt-3 text-sm font-medium text-[#b6349a]" role="status">{passwordStatus}</p>}
      <p className="mt-8 border-t border-[#eee] pt-5 text-sm text-[#555]">Bu sitede iki aşamalı doğrulama etkinleştirme özelliği yoktur. Google ile giriş güvenliği için Google hesabınızda iki adımlı doğrulamayı açın.</p>
    </section>}
