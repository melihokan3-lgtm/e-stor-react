...
export default function AccountSettings() {
  ...
  const handlePasswordUpdate = async (): Promise<void> => {
    ...
    if (error) {
      setPasswordStatus("Password update failed. Please try again.");
      return;
    }
    setNewPassword("");
  ...
  return <div className="w-full space-y-5">
    <SeoMeta title={translate("Hesap Ayarları | E-Storee")} description={translate("Manage your E-Storee account, security, notification and privacy settings.")} canonicalPath="/profile/settings" robots="noindex,nofollow" />
    ...
    {activeTab === "security" && <section className={sectionClass}>
      ...
      <p className="mt-8 border-t border-[#eee] pt-5 text-sm text-[#555]">{translate("This site does not support enabling two-factor authentication. For safer Google sign-in, enable 2-Step Verification in your Google Account.")}</p>
    </section>}
