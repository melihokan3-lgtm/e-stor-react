...
export default function Navbar() {
  ...
  const avatarSrc =
    user?.image?.trim() ||
    "https://ui-avatars.com/api/?name=User&background=b6349a&color=fff";
  ...
  useEffect(() => {
    const handleCategoriesOutsideClick = (
  ...
  const handleSearch = (termToSearch: string = searchTerm): void => {
    if (termToSearch) {
      const updatedSearches = [
        termToSearch,
        ...pastSearches.filter((s) => s !== termToSearch),
      ].slice(0, 5);
  ...
  return (
    <>
      <nav className="relative z-20 flex flex-wrap items-center justify-between gap-[15px] border-b border-[var(--border-light)] px-5 py-4 md:flex-nowrap md:justify-start md:px-5">
        ...
        <div
          className="relative order-2 hidden md:ml-2 md:block"
          ref={categoriesRef}
        >
          <button
            type="button"
          ...
          {categoriesOpen && (
            <div
              className="absolute left-0 top-[calc(100%+12px)] z-40 grid w-[420px] grid-cols-2 gap-1 rounded-2xl border border-[#eee] bg-white p-3 shadow-[0_16px_35px_rgba(0,0,0,0.14)]"
              role="menu"
            >
        ...
        <div className="order-2 md:order-3" ref={menuRef}>
          ...
          <div
            className={
              menuOpen
                ? "absolute left-5 right-5 top-[84px] z-30 flex max-h-[calc(100vh-100px)] flex-col gap-4 overflow-y-auto rounded-[18px] bg-white p-4 shadow-[0_12px_35px_rgba(0,0,0,0.15)] md:hidden"
                : "hidden"
            }
          >
            <div className="grid grid-cols-2 gap-3 border-b border-[#eee] pb-4">
              <Link
                ...
                className="flex min-h-[72px] flex-col items-center justify-center gap-2 rounded-[14px] bg-[#fef5fd] text-sm font-semibold text-[#333] transition hover:bg-[#fbe8f7]"
              >
                <span className="relative">
                  <img src="/img/icon/Buy.svg" alt="" aria-hidden="true" width={24} height={24} className="h-6 w-6" />
                  <span className="absolute -right-3 -top-2 text-[11px] font-bold text-[#b6349a]">
                    {totalItems}
                  </span>
                </span>
              ...
              {isLoggedIn && user ? (
                <Link
                  to="/profile/details"
                  onClick={() => setMenuOpen(false)}
                  className="flex min-h-[72px] flex-col items-center justify-center gap-2 rounded-[14px] bg-[#f8f7f8] text-sm font-semibold text-[#333] transition hover:bg-[#f1edf1]"
                >
                  <img
                    src={avatarSrc}
                    alt=""
            ...
            <button
              ...
              className="flex items-center gap-3 rounded-[14px] border border-[#eee] px-4 py-3 text-left text-sm font-medium text-[#333] transition hover:border-[#b6349a] hover:text-[#b6349a]"
            >
              <img src="/img/icon/Location.svg" alt="" aria-hidden="true" width={20} height={20} className="h-5 w-5" />
              <span className="min-w-0 flex-1 truncate">
                {displayLocation || "Select delivery location"}
              </span>
            ...
            <div className="absolute left-0 top-[calc(100%+8px)] z-30 w-full overflow-hidden rounded-2xl border border-[#e5e7eb] bg-white py-2 shadow-[0_10px_25px_rgba(0,0,0,0.1)]">
              <ul className="m-0 list-none p-0">
                {pastSearches.map((search) => (
                  <li
                    className="flex cursor-pointer items-center justify-between px-4 py-2 text-sm hover:bg-[#fff5fc]"
                    key={search}
                    onClick={() => {
        ...
        <div className="order-3 ml-2 hidden items-center gap-4 md:ml-auto md:mr-0 md:flex">
          ...
          >
            <div className="relative flex items-center">
              ...
              {totalItems > 0 && (
                <p className="absolute -right-2 -top-2 min-w-3 text-center text-[11px] font-semibold text-[#d73f98]">
                  {totalItems}
                </p>
              )}
            </div>
