    ...
    <div className={`flex items-center justify-between py-[30px] max-md:items-start max-md:gap-4 ${last ? "" : "border-b border-[#f2f2f2]"}`}>
      {editing ? (
        <div className="flex w-full flex-col gap-3">
          <span className="text-[15px] font-bold text-[#111]">Edit {label}</span>
          ...
          {error && <span className="text-xs font-medium text-red-500" role="alert">{error}</span>}
          <div className="mt-1 flex items-center gap-2">
            ...
            <button type="button" className="inline-flex items-center rounded-lg bg-gray-100 px-[14px] py-2 text-[13px] font-semibold text-gray-600 transition hover:bg-gray-200 hover:text-gray-900" onClick={onCancel}>Cancel</button>
          </div>
        </div>
      ) : (
        <>
