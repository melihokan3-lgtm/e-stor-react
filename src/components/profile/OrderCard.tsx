...
export default function OrderCard({ order, onEditAddress }: OrderCardProps) {
  ...
  return (
    <div className="rounded-2xl border border-[#eee] bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.03)]">
      {order.isDemo && <p className="mb-4 rounded-xl bg-amber-50 px-3 py-2 text-sm font-medium text-amber-900">Test işlemi — gerçek ödeme alınmadı, ürün gönderilmeyecek.</p>}
      <div className="mb-4 flex items-center justify-between gap-3 border-b border-[#f2f2f2] pb-4">
        <div className="flex items-center gap-3">
          <span className="text-sm font-bold text-[#111]">Order {order.isDemo ? String(order.id).replace(/^(?:demo_|test_)/, "T-") : order.id}</span>
      ...
      <div className="mb-4 flex items-start justify-between gap-4 rounded-xl bg-[#faf9fa] p-4 max-sm:flex-col">
        ...
        <div className="shrink-0">
          {isEditable ? (
            ...
            <span className="text-xs font-semibold text-amber-800">Test işlemi — gönderim yok</span>
          ) : (
