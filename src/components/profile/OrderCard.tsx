...
export default function OrderCard({ order, onEditAddress }: OrderCardProps) {
  ...
  return (
    <div className="rounded-2xl border border-[#eee] bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.03)]">
      {order.isDemo && <p className="mb-4 rounded-xl bg-amber-50 px-3 py-2 text-sm font-medium text-amber-900">{translate("Test işlemi — gerçek ödeme alınmadı, ürün gönderilmeyecek.")}</p>}
        ...
        <div className="shrink-0">
          {isEditable ? (
            <button
              ...
              title={translate("Kargoya verilmediği için adresi değiştirebilirsiniz")}
            >
            ...
            <span className="text-xs font-semibold text-amber-800">{translate("Test işlemi — gönderim yok")}</span>
          ) : (
            <span className="inline-flex items-center gap-1.5 text-xs text-[#6b7280]" title={translate("Sipariş kargoya verildiği veya teslim edildiği için adres değiştirilemez")}>
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <span className="text-xs text-[#999]">{translate("Qty: ")}{item.qty} {translate(" &middot; $")}{Number(item.price || 0).toFixed(2)}</span>
              </div>
            </Link>
          );
        })}
